...
CREATE DATABASE IF NOT EXISTS `G` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `G`;
...
CREATE TABLE `orders` (
  ...
  CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`item_id`) REFERENCES `grocery_items` (`item_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
BEGIN
    ...
    -- Check if the item_name exists in the grocery_items table
    IF (SELECT COUNT(*) FROM grocery_items WHERE name = p_item_name) > 0 THEN
...
-- Function to get total order price for an order_id
DELIMITER ;;
...
BEGIN
    ...
    SET v_item_id = (SELECT item_id FROM grocery_items WHERE name = p_food_item);
